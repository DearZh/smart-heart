...
public abstract class AbstractQueuedSynchronizer
    ...
    /**
     * The synchronization state. 线程安全的同步变量
     */
    ...
    private Node enq(final Node node) {
        ...
        for (; ; ) {
            //tail 尾结点
            Node t = tail;
            if (t == null) { // 如果尾结点 t 为 空，则 必须初始化  Must initialize
                //此处compareAndSetHead判断，当前head是否是null，是，则赋值为 new Node()  及 head = new Node() ； cas 操作具体可以看compareAndSetHead() 代码
                if (compareAndSetHead(new Node()))
                    //然后再把 head的值赋值给 tail，此时 t = tail = head = new Node() ； 及 当前整个一个Node节点已经都初始化完成了，此时 tail = head；
                    tail = head;
            } else {
                //进行二次循环，此时t 尾结点不再为空，将t的值赋值给node.prev； 也就是 node 的前置节点 prev 绑定对应的 tail节点；
                node.prev = t;
                //cas判断当前tail属性的值是否是 t，是，则将 node 赋值给tail； 此时 tail = node ；完成入队操作，及 原有的tail节点变成了当前node节点的prev前置节点，而该node节点则赋值给了tail成为了当前AQS的尾结点；
                if (compareAndSetTail(t, node)) {
                    //但是还差一点，原有的tail节点变成了当前node节点的prev前置节点；那么还需要将node节点变更为原有tail节点的next后置节点；所以最后一步  t.next = node
                    t.next = node; //将原有的 tail 节点的next下置节点属性 关联为 node； 此时整个入队操作全部完成，
                    return t;
                }
                //此时可能会有一丝疑问，cas的操作：compareAndSetTail(t, node) 将当前的node节点赋值为了tail节点，此时的赋值会不会存在传导性？及 t = tail的引用，那么此时 node赋值给了 tail，此时t 是不是也变更为了 node？
                //及 t = tail = node ?  此处说明一点，cas的操作是直接使用Unsafe进行内存位的赋值，及原有的 tail 的引用的内存地址会直接变更为 node的内存位，但是t 原有所引用的tail的内存位是不变的，还是直接指向原有的tail
                //内存位；所以，此处cas的赋值不会存在传导性；否则这整个一个链路就乱了。
            }
        }
    }
    ...
    private Node addWaiter(Node mode) {
        ...
        //当前对列未初始化，则调用 enq() 方法进行队列初始化，并将当前 node节点进行入队操作
        enq(node); //无锁算法的标准for循环尝试
        return node;
    }
    ...
    /**
     * 取消正在进行的获取尝试 Cancels an ongoing attempt to acquire.
     *
     * @param node the node
     */
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
             ...
             * indicate retry.
             * ws > 0 表示当前该节点线程已被取消，重新循环获取前一个ws 不大于0的节点，然后赋值给当前node节点
             */
            do {
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                //如果当前节点的上一个节点是 head 且 tryAcquire() 可以获取到锁
                if (p == head && tryAcquire(arg)) {
                    //更改当前节点为 head，此时该节点的thread设置为null，prev也设置为空； tryAcquire() 方法执行的时候，已经将当前线程赋值给exclusiveOwnerThread了
                    setHead(node);
                    //设置当前p节点 next 为空 ，便于gc回收 p 节点
                    p.next = null; // help GC
                    failed = false;
                    //return false，此时获取锁成功
                    return interrupted;
                }
    ...
    public final void acquire(int arg) {
        //tryAcquire(arg) 尝试获取锁，该方法为子类所实现的自定义业务方法；以ReentranLock为例 1、判断当前线程是否能获取到锁，2、判断当前线程是否可以重入锁
        //addWaiter(Node.EXCLUSIVE) ：获取锁失败，则执行addWaiter(Node.EXCLUSIVE) 将当前线程加入等待队列；
        //acquireQueued()：当前线程入队后，则执行acquireQueued()方法，再次自循环加cas判断，判断当前节点的上节点prev是否是head，
        //如果是，则再次执行tryAcquire() 判断当前该线程是否可以获取到锁，如果还是不能，则直接将当前线程的上一个prev节点的waitStatus状态设置为SIGNAL(待唤醒状态) ，
        //然后将当前节点的线程进行挂起,并发出线程中断信令进行线程中断；

        //可以看到此处AQS源码还是很谨慎的，在执行acquireQueued() 线程挂起操作时，依然判断了当前线程是否可以获取到锁，因为的确可能存在，在线程进行挂起时，
     ...
     * @since 1.7
     */
    //判断是否存在 在排队的前节点，存在则返回 true，否则false；
